...
export async function POST(request: Request) {
  try {
    const { name, phone, email, preferredCar, budgetRange, message } = await request.json();
    ...
    const { error: dbError } = await supabase.from("quotes").insert({
      ...
      phone,
      email,
      preferred_car: preferredCar || null,
    ...
    try {
      await resend.emails.send({
        ...
        html: `
          ...
          <p><strong>Preferred Car:</strong> ${preferredCar || "Not specified"}</p>
